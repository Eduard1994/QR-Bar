...
@IBDesignable open class SnakePageControl: UIView {
    ...
    // MARK: - Appearance
    @IBInspectable open var activeTint: UIColor = UIColor.black {
        didSet {
            activeLayer.backgroundColor = activeTint.cgColor
        }
    }
    
    @IBInspectable open var inactiveTint: UIColor = UIColor.mainGray {
        didSet {
            inactiveLayers.forEach() { $0.backgroundColor = inactiveTint.cgColor }
        }
    }
    
    @IBInspectable open var previousTint: UIColor = UIColor.mainGray {
        didSet {
            inactiveLayers.first?.backgroundColor = previousTint.cgColor
            inactiveLayers[1].backgroundColor = previousTint.cgColor
        }
    }
    ...
    fileprivate func layoutActivePageIndicator(_ progress: CGFloat) {
        ...
        inactiveLayers.first?.backgroundColor = activeTint.cgColor
