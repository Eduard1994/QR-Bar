...
@IBDesignable open class SegmentedControl: UIControl {
    ...
    public var segments: [SegmentedControlSegment] {
        didSet {
            applySegments()
        }
    }
    ...
    private static var defaultOptions: [Option] = [
        .backgroundColor(.mainBlack),
        .indicatorViewBackgroundColor(.mainBlue)
    ]
    
    // MARK: Initialization
    /// Initializes a new `SegmentedControl` with the parameters passed.
    ///
    /// - Parameters:
    ///   - frame: The frame of the control.
    ///   - segments: The segments to configure the control with.
    ///   - index: The initially selected index. Passing `SegmentedControl.noSegment` sets the index to `-1` and hides the
    ...
    public init(frame: CGRect,
        ...
        setOptions(SegmentedControl.defaultOptions)
    ...
    ///
    /// - Parameters:
    ///   - index: The new index. Passing `SegmentedControl.noSegment` sets the index to `-1` and hides the indicator view.
